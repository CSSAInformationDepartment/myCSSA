...
class saveBlog (LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if blogId == NEW_BLOG:
            newB = BlogModels.BlogDescription(
                blogTitle=request.POST["blogTitle"], 
                blogReads = 0, 
                blogOpenToPublic = request.POST["blogOpenToPublic"])
            newB.save()
            blog = newB
            blogId = newB.blogId
        else:
            currentBlog = BlogModels.BlogDescription.objects.get(blogId=blogId)
            if currentBlog:
                currentBlog = BlogModels.BlogDescription(
                    blogId = blogId, 
                    blogTitle=request.POST["blogTitle"], 
                    blogReads = currentBlog.blogReads, 
                    blogOpenToPublic = request.POST["blogOpenToPublic"])
                currentBlog.save()
                blog = currentBlog
            else:
                # currentBlog不存在 blogId有问题
                return JsonResponse({
                    'success': False,
                    'status': '400',
                    })

        blogMainContent = json.loads(request.POST["contents"])

        # store all 

        if contentId == NEW_CONTENT:
            pass
