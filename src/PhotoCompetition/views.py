...
class CandidateSubmissionView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        '''
        prev_submission = models.Submission.objects.filter(
            Q(submissionUserId__user=request.user.id) & Q(submissionTime__date__gt=datetime.date(2019, 8, 1)))
...
#     '''
#     PhotoCompetition App - CandidateSubmissionView
#     Written by Tianzhi (Eric) - 2019

#     This view is for presenting the awarded photo form and retriving candidate's submission data
#     '''
#     login_url = '/hub/login/'
#     template_name = 'PhotoCompetition/photoSubmit1.html'
#     form_class = forms.CandidateSubmissionForm
...
class SubmissionListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["filter"] = filters.SubmissionFilter(
...
class SubmissionDetailView(LoginRequiredMixin, PermissionRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['UserInfo'] = UserProfile.objects.filter(
            pk=self.object.submissionUserId.pk).first()
        context['AccountInfo'] = User.objects.get(
            pk=self.object.submissionUserId.pk)
        return context
...
class SubmissionSelectionControlAPI(APIView):
    ...
    queryset = models.ApprovedSubmission.objects.none()
    ...
    def get(self, request, format=None):
        ...
        approved_submission = models.ApprovedSubmission.objects.filter(
            submission__pk=pk).first()
    ...
    def post(self, request, format=None):
        serializer = serializers.SubmissionSelectionControlSerializers(
            data=request.data)
...
class VoteSubmissionControlAPI(APIView):
    ...
    def check_duplicate_votes(self, key: str):
        ...
        count: int = get_previous_path_ip_count_by_hour(
            self.request, path=self.request.path_info, method='POST')
        voter_ip = get_request_IPv4(self.request)
        obj = None
        if count >= 3:
            raise ValidationError(
                _('You have exceeded the daily voting limit'))
        ...
        if obj:
            raise ValidationError(
                _('You cannot vote the same candidate within 24 hours'))
    ...
    def post(self, request, format=None):
        ...
        try:
            obj_confirm = models.ApprovedSubmission.objects.get(
                submission__pk=votable_submission_id)
        except:
...
class resultDisplay(View):

    def get(self, request, *args, **kwargs):
        return render(self.request, template_name='PhotoCompetition/result_display.html')
