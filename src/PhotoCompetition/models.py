...
class Submission(models.Model):
    ...
    submissionId = models.UUIDField(
        primary_key=True, default=uuid.uuid4, editable=False)
    submissionUserId = models.ForeignKey(
    ...
    deviceType = models.CharField(verbose_name=_(
        "设备类型"), choices=DEVICE_CHOICE, max_length=30, default="MobilePhone", null=True)
    categoryType = models.CharField(verbose_name=_(
        "题材类型"), choices=CATEGORY_CHOICE, max_length=30, default="Nature", null=True)
    themeType = models.CharField(verbose_name=_(
        "主题类型"), choices=THEME_CHOICE, max_length=30, default="Beauty", null=True)
    upload_photo = SorlImageField(verbose_name=_(
        "上传作品"), null=True, upload_to=_GetUserDir)
...
class ApprovedSubmission(models.Model):
    ...
    submission = models.ForeignKey(
        Submission, verbose_name=_("作品"), on_delete=models.CASCADE)
...
class SubmissionVoting(models.Model):
    ...
    voter = models.ForeignKey(
        adminModel.UserProfile, on_delete=models.DO_NOTHING, default=None, blank=True, null=True)
    user_IPv4 = models.GenericIPAddressField()
    votable_submission = models.ForeignKey(
        ApprovedSubmission, on_delete=models.CASCADE)

    def _add_aggregate_votes(self, created=False):
        if created:
            target_transaction = ApprovedSubmission.objects.get(
                pk=self.votable_submission.pk)
