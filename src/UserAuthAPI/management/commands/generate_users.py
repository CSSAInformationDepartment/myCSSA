...
def create_user():
    ...
    def _email_gen(name:str):
        ENDFIX = '@gmail.com'
        random_chars = ''.join(random.choice(string.ascii_letters) for i in range(0,4))
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--num_of_users', nargs='?', type=int, const=10, default=10)
    

    def handle(self, *args, **options):
        if settings.DEBUG == False:
            raise CommandError('This command is forbidden to use in Production Mode')
        num_of_users = options['num_of_users']
        if num_of_users < 0:
            raise CommandError('The number of users you want to create must larger than 0')
        
        for i in range(0, num_of_users):
            self.stdout.write(self.style.SUCCESS((f"The {i+1} user is created, named {create_user()}")))
        return 0
