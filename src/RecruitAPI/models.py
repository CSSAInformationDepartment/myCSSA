...
class JobList(models.Model):
    ...
    dept = models.ForeignKey(
        adminModel.CSSADept, verbose_name="部门", on_delete=models.CASCADE)
    timeOfCreate = models.DateTimeField(auto_now_add=True)
    jobName = models.CharField(
        verbose_name="职位名称", max_length=50, default=None)
    description = models.CharField(
        verbose_name="职位介绍", max_length=400, default=None)
    dueDate = models.DateTimeField(
        verbose_name="截止日期", default=None, null=True)

    disabled = models.BooleanField(default=False, blank=True, null=True)


class Resume(models.Model):
    CVId = models.UUIDField(
        primary_key=True, default=uuid.uuid4, editable=False)
    timeOfCreate = models.DateTimeField(auto_now_add=True)

    jobRelated = models.ForeignKey(
        JobList, verbose_name="相应职位", on_delete=models.CASCADE)
    user = models.ForeignKey(
        adminModel.User, on_delete=models.DO_NOTHING, default=None, blank=True, null=True)

    reason = models.CharField(
        verbose_name="申请原因", max_length=400, default=None)
    hobby = models.CharField(verbose_name="兴趣爱好", max_length=400, default=None)
    eduBackground = models.CharField(
        verbose_name="教育背景", max_length=400, default=None, blank=True, null=True)
    inSchoolExp = models.CharField(
        verbose_name="校内经历", max_length=400, default=None)
    outSchoolExp = models.CharField(
        verbose_name="校外经历", max_length=400, default=None, blank=True, null=True)
    additionMsg = models.CharField(
        max_length=400, default=None, blank=True, null=True)

    attachment = models.FileField(
        default=None, null=True, blank=True, upload_to=_GetUserDir)
...
class InterviewTimetable(models.Model):
    ...
    resume = models.ForeignKey(
        Resume, on_delete=models.DO_NOTHING, default=None)
