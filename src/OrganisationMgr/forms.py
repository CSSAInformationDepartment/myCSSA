...
from UserAuthAPI.models import CSSACommitteProfile, User, UserProfile
...
class BindingMembershipCardForm(forms.ModelForm):
    ...
    def clean(self, *args, **kwargs):
        ...
        prev_member = UserProfile.objects.filter(
            membershipId=membership_Id).first()
        legacy_member = LegacyUsers.objects.filter(
            membershipId=membership_Id).first()
...
class UserProfileEditForm(forms.ModelForm):
    ...
    class Meta:
        ...
        model = UserProfile
        exclude = ('avatar', 'infocardBg', 'identiyConfirmed',
                   'isValid', 'joinDate', 'dateOfBirth', 'membershipId')
    ...
    def clean(self, *args, **kwargs):
        ...
        duplicated_student_id = UserProfile.objects.exclude(
            user__pk=user_id).filter(studentId=student_id).first()
        if duplicated_student_id:
            raise ValidationError(
                _(mark_safe('<li>该学生卡号已被占用</li>')), code='Duplicated Student Id')
...
class UserEditForm(forms.ModelForm):
    ...
    def clean(self, *args, **kwargs):
        ...
        duplicated_telNumber = User.objects.exclude(
            email=email).filter(telNumber=telNumber).first()
        if duplicated_telNumber:
            raise ValidationError(
                _(mark_safe('<li>该手机号已被他人注册</li>')), code='Duplicated tel number')
