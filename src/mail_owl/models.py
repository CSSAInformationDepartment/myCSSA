...
class MailDraft(models.Model):
    ...
    author = models.ForeignKey(
        UserModels.UserProfile, on_delete=models.DO_NOTHING)
...
class MailAttachments(models.Model):
    ...
    id = models.UUIDField(primary_key=True, editable=False, default=uuid4)
    related_mail = models.ForeignKey(
        MailDraft, verbose_name=_("关联邮件"), on_delete=models.CASCADE)
    date_created = models.DateTimeField(
        verbose_name=_("创建时间"), auto_now_add=True)
    file_name = models.CharField(verbose_name=_("文件名"), max_length=256)
    file_type = models.CharField(verbose_name=_(
        "文件类型"), max_length=10, choices=FILE_TYPE)
    file = models.FileField(verbose_name=_(
        "文件"), upload_to="public/email/attachments/%Y/%m/%d/")
...
class MailQuene(models.Model):
    ...
    date_scheduled = models.DateTimeField(
        verbose_name=_("设定发送时间"), default=sys_now_time)
    receiver = models.EmailField(verbose_name=_("收件人"))
    title = models.CharField(verbose_name=_("标题"), max_length=100)
    mail_text = models.TextField(verbose_name=_(
        "邮件正文"), default=None, blank=True, null=True)
    mail_html = models.TextField(verbose_name=_(
        "邮件HTML"), default=None, blank=True, null=True)
    priorty = models.IntegerField(
        verbose_name=_("优先级"), choices=PRIORITY, default=1)
    state = models.IntegerField(verbose_name=_(
        "发送状态"), choices=STATUS, default=0)
    disabled = models.BooleanField(default=False)
