...
class Event (models.Model):
    ...
    eventName = models.CharField(verbose_name=_(
        "活动名称"), max_length=40, unique=True)
    ...
    mainVisual = models.ImageField(verbose_name=_(
        "主视觉图"), upload_to='uploads/usrImage/eventMainVisual', default=None, null=True)

    hasMaxAttendent = models.BooleanField(
        verbose_name=_("是否有人数上限?"), default=False)
    maxAttendent = models.IntegerField(verbose_name=_(
        "人数上限"), default=0, blank=True, validators=[MaxValueValidator(1000),])
    address = models.CharField(verbose_name=_(
        "地址"), max_length=200, default=None, null=True)
    venue = models.CharField(verbose_name=_(
        "场馆名称"), max_length=50, default=None, null=True)

    isMemberOnly = models.BooleanField(
        verbose_name=_("是否为CSSA会员限定活动?"), default=False)

    isFree = models.BooleanField(verbose_name=_("是否为免费活动?"), default=True)
    price = models.DecimalField(verbose_name=_(
        "门票价格"), max_digits=10, decimal_places=2, default=0.0)

    # 活动主办方
    eventBy = models.ForeignKey(EventUndertaker, verbose_name=_(
        "活动主办方"), on_delete=models.PROTECT)

    # 活动类型
    eventTypes = models.ForeignKey(
        EventType, verbose_name=_("活动类型"), on_delete=models.DO_NOTHING)
    ...
    DisplayArticleType = models.CharField(verbose_name=_(
        "文章显示来源"), choices=ArticleSource, max_length=30, default=None, null=True)
    # 相关推文
    relatedArticles = models.ForeignKey(BlogModel.Blog, verbose_name=_(
        "相关介绍文章"), on_delete=models.PROTECT, default=None, blank=True, null=True)
    # 相关微信推文（外链）
    WechatArticleUrl = models.CharField(verbose_name=_(
        "微信公众号文章链接"), max_length=500, default=None, null=True, blank=True)
    WechatQRcode = models.ImageField(verbose_name=_(
        "微信二维码"), upload_to='uploads/usrImage/eventWechatQRcode', default=None, null=True, blank=True)

    # 小程序中用到的字段
    retrospectArticleLink = models.URLField(
        verbose_name=_("活动回顾（往期活动）文章链接"), default=None, null=True)
...
class AttendEvent(models.Model):
    ...
    '''
    attendedId = models.UUIDField(
        primary_key=True, default=uuid.uuid4, editable=False)

    attendedEventId = models.ForeignKey(Event, on_delete=models.CASCADE)
    attendedUserId = models.ForeignKey(
        adminModel.UserProfile, on_delete=models.DO_NOTHING)
...
class EventAttendentInfoForm(models.Model):
    ...
    event = models.ForeignKey(
        Event, on_delete=models.CASCADE, verbose_name=_("绑定新活动"))
