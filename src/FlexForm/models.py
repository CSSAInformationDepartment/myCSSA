...
class FlexForm(models.Model):
    ...
    name = models.CharField(verbose_name=_(
        "表单标题"), max_length=100, unique=True)
...
class FlexFormField(models.Model):
    ...
    field_type = models.CharField(verbose_name=_(
        "字段类型"), max_length=10, choices=typeChoice)
    max_len = models.IntegerField(verbose_name=_(
        "字数上限"), validators=[MaxValueValidator(2000),])
...
class FlexFormData(models.Model):
    ...
    file_input = models.FileField(
        default=None, null=True, blank=True, upload_to=_GetFormUploadsDir)
